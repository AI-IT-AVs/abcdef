...
from posdiff.modules.transformer.vanilla_transformer import TransformerLayer

from posdiff.modules.transformer.pp_transformer import PPTransformerLayer
from posdiff.modules.transformer.ppcross_transformer import PPCrossTransformerLayer
import torch
...
class PPConditionalTransformer(nn.Module):
    def __init__(
        self,
        blocks,
        d_model,
        ...
        super(PPConditionalTransformer, self).__init__()
        self.blocks = blocks
        layers = []
        for block in self.blocks:
            _check_block_type(block)
            ...
            if block == 'self':
                layers.append(PPTransformerLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
            else:
                layers.append(PPCrossTransformerLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
        self.layers = nn.ModuleList(layers)
        self.return_attention_scores = return_attention_scores
        self.parallel = parallel
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                ...
                if self.parallel:
                    ...
                    new_feats0, scores0 = self.layers[i](feats0, feats1, embeddings0, embeddings1, memory_masks=masks1)
                    new_feats1, scores1 = self.layers[i](feats1, feats0, embeddings1, embeddings0, memory_masks=masks0)
                    feats0 = new_feats0
                    feats1 = new_feats1
                else:
