...
def point_to_node_partition(
    ...
    if sq_dist_mat.size(1) >= point_limit:
        node_knn_indices = sq_dist_mat.topk(k=point_limit, dim=1, largest=False)[1]  # (M, K)
    else:
        point_limit_short = sq_dist_mat.size(1)
        node_knn_indices = sq_dist_mat.topk(k=point_limit_short, dim=1, largest=False)[1]
  

    node_knn_node_indices = index_select(point_to_node, node_knn_indices, dim=0)  # (M, K)
    point_limit_min=min([sq_dist_mat.size(1), point_limit])
    node_indices = torch.arange(nodes.shape[0]).cuda().unsqueeze(1).expand(-1, point_limit_min)  # (M, K)
