...
from posdiff.engine.base_trainer import BaseTrainer
from posdiff.utils.torch import to_cuda
...
class EpochBasedTrainer(BaseTrainer):
    ...
    def train_epoch(self):
        ...
        for iteration, data_dict in tqdm.tqdm(enumerate(self.train_loader)):
    ...
    def inference_epoch(self, inference_epoch, txt):
        self.set_eval_mode()
        ...
        txt.append(f'epoch {inference_epoch:.4f}')
        txt.append('---------------------------------------------------------')
    ...
    def run(self):
        ...
        while self.epoch < self.max_epoch:
            self.epoch += 1
            self.train_epoch()
            self.inference_epoch(self.epoch, txt)
